...
const BASE_URL = "https://logistic-backend-nu.vercel.app/api/peminjaman";
...
const peminjamanAPI = {
  ...
  getAll: async (params = {}) => {
    try {
      ...
      console.log(
        `📋 Retrieved ${result.data?.length || 0} peminjaman records`
      );
      return result;
    } catch (error) {
  ...
  delete: async (id) => {
    try {
      console.log(`🗑️ API: Deleting peminjaman: ${id}`);
      ...
      if (result.success) {
        ...
        // Jika endpoint tidak mengembalikan success field
        return {
          success: true,
          message: result.message || "Peminjaman deleted successfully",
        };
      }
    } catch (error) {
      console.error(`❌ API: Error deleting peminjaman ${id}:`, error);
      return {
        success: false,
  ...
  search: async (searchTerm, params = {}) => {
    try {
      ...
      return {
        success: false,
        message:
          error.message || `Gagal mencari peminjaman untuk "${searchTerm}"`,
        data: [],
      };
    }
  },
  ...
  getByStatus: async (status, params = {}) => {
    try {
      ...
      const result = await fetchAPI(url);
      console.log(
        `📊 Found ${result.data?.length || 0} peminjaman with status ${status}`
      );
      return result;
    } catch (error) {
      console.error(
        `❌ Error fetching peminjaman with status ${status}:`,
        error
      );
      return {
        success: false,
        message:
          error.message || `Gagal memuat peminjaman dengan status ${status}`,
        data: [],
      };
    }
  },
  ...
  bulkUpdateStatus: async (ids, status) => {
    try {
      console.log(
        `🔄 Bulk updating ${ids.length} records to status: ${status}`
      );
  ...
  canDeleteBarang: async (barangId) => {
    try {
      ...
      const peminjamanResult = await fetchAPI(
        `${BASE_URL}/barang/${barangId}/active`
      );

      if (
        peminjamanResult.success &&
        peminjamanResult.data &&
        peminjamanResult.data.length > 0
      ) {
        return {
          success: true,
          canDelete: false,
  ...
  getActivePeminjamanByBarang: async (barangId) => {
    try {
      ...
      return {
        ...
        message:
          error.message ||
          `Gagal memuat peminjaman aktif untuk barang ${barangId}`,
        data: [],
      };
    }
  },
  ...
  exportData: async (params = {}) => {
    try {
      ...
      link.download = `peminjaman-export-${
  ...
  utils: {
    ...
    calculateTotalCost: (hargaPerHari, jumlahPinjam, lamaPinjam) => {
      return (
        (parseFloat(hargaPerHari) || 0) *
        (parseInt(jumlahPinjam) || 1) *
        (parseInt(lamaPinjam) || 1)
      );
    },
  },
};
